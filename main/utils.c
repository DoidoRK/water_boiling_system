...
command_data_packet_t json_to_command_data_packet(const char *json_str) {
    ...
    if (cJSON_IsObject(system_settings)) {
        packet.system_settings.water_supply_volume = cJSON_GetObjectItem(system_settings, "water_supply_volume")->valueint;
        packet.system_settings.boiling_tank_volume = cJSON_GetObjectItem(system_settings, "boiling_tank_volume")->valueint;
